defmodule UAParser.Index.RequirementMiner do
  ...
  # Derives a *requirement* from a regex source: a literal any matching
  # string must contain. It runs at compile time only (see
  # `UAParser.Index.Requirements`), in two passes: `tokenize/1` reads the
  # regex syntax into a small token tree, and `analyze/1` finds the longest
  # literal that must be present, plus any alternation sets.
  #
  # A requirement is one of:
  #
  #   * `{:all, literal}` - `literal` must be present
  #   * `{:any, literals}` - at least one of `literals` must be present
